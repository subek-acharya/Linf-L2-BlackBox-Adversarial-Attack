...
class SurFree():
    ...
    def __call__(self, model, X, labels, starting_points=None, **kwargs):
        ...
        for step_i in range(self._steps):
            ...
            self.best_advs = torch.where(
                utils.atleast_kdim(is_success * self._images_finished.logical_not(), len(best_candidates.shape)), 
                best_candidates, 
                self.best_advs
                )
    ...
    def _alpha_binary_search(self, function_evolution, lower):    
        ...
        while step < self._BS_max_iteration and over_gamma.any(): 
            ...
            mid = torch.where(
                utils.atleast_kdim(torch.logical_and(mid_bound != 0, over_gamma), len(self.X.shape)),
            ...
            mid_opp = torch.where(
                utils.atleast_kdim(torch.logical_and(check_opposite, over_gamma), len(mid.shape)),
                function_evolution(-mid_bound),
...
class Basis:
    def __init__(self, originals: torch.Tensor, random_noise: str = "normal", basis_type: str = "dct", **kwargs):
        ...
        """
        self.X = originals
        self._f_dct2 = lambda a: dct.torch_dct.dct_2d(a)
        self._f_idct2 = lambda a: dct.torch_dct.idct_2d(a)
