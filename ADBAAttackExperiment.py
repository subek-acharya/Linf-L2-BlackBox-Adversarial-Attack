...
class ADBAAttackExperiment:
    def __init__(
        self, experiments_config, epsilon_max, query_limit=10000, total_samples=1000, init_dir=1, offspring_n=2, binary_mode=0, channels=1,
    ):
    ...
    def _save_samples(self, adv_loader, model, model_name, dataset_path, n_save=20):
        base_dir = os.path.join("adv_samples", "adba", model.__class__.__name__)
        ...
        with open(filepath, "a", encoding="utf-8") as f:
            if eps_key not in self.headers_written:
                f.write(
                    "\n"
                    + "=" * 10
                    + f" ADBA ATTACK FINAL RESULTS eps={eps_key}/255 "
                    + "=" * 10
                    + "\n"
                )
