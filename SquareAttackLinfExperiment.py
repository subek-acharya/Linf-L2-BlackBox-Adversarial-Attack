...
class SquareAttackLinfExperiment:
    def __init__(
        self, experiments_config, epsilon_max, n_iters=40000, p_init=0.8, n_classes=2, targeted=False, loss_type="cross_entropy", total_samples=1000,
    ):
    ...
    def run_all(self, batch_size=64):
        for model_name, config in self.experiments.items():
            print(
                f"Model: {model_name}, Dataset: {config['dataset_path']}, Epsilon Max: {self.epsilon_max}, N_Iters: {self.n_iters}, Total Samples: {self.total_samples}"
            )
            ...
            result_key = model_name + f"_eps={int(self.epsilon_max * 255)}/255"
            self.results[result_key] = acc

            self._append_result(result_key, acc)
    ...
    def _save_samples(self, adv_loader, model, model_name, dataset_path, n_save=20):
        base_dir = os.path.join("adv_samples", "square_linf", model.__class__.__name__)
        ...
        with open(filepath, "a", encoding="utf-8") as f:
            if eps_key not in self.headers_written:
                f.write(
                    "\n"
                    + "=" * 10
                    + f" SQUARE ATTACK LINF FINAL RESULTS eps={eps_key}/255 "
                    + "=" * 10
                    + "\n"
                )
